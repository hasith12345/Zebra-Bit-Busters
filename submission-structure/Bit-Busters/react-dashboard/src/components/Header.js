...
const LiveIndicator = ({ connectionInfo }) => (
  <div className="flex items-center">
    <span
      className={`inline-block w-3 h-3 rounded-full mr-2 shadow-glow ${
        connectionInfo.text === "LIVE"
          ? "bg-green-400 animate-pulse-slow"
          : connectionInfo.text === "OFFLINE"
          ? "bg-red-400"
          : "bg-yellow-400 animate-pulse"
      }`}
    ></span>
    <span className={`${connectionInfo.color} font-semibold text-sm`}>
      {connectionInfo.text}
    </span>
  </div>
);
...
const Header = ({
  lastUpdated,
  connectionInfo = { color: "text-green-400", text: "LIVE" },
}) => {
  return (
    <div className="text-center bg-glass-light backdrop-blur-xl border border-white border-opacity-20 p-8 rounded-3xl mb-6 shadow-glass animate-fade-in">
      ...
      <div className="flex items-center justify-center space-x-4 mb-4">
        <span className="text-sentinel-accent font-semibold">
          Team Bit-Busters
        </span>
        <div className="w-1 h-1 bg-gray-400 rounded-full"></div>
        <LiveIndicator connectionInfo={connectionInfo} />
        <div className="w-1 h-1 bg-gray-400 rounded-full"></div>
