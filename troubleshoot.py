...
class SentinelTroubleshooter:
    ...
    def check_python_version(self):
        ...
        if sys.version_info >= (3, 7):
            ...
            print(
                f"❌ Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro} - Need 3.7+")
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    if result == 0:
                        ...
                        self.solutions.append(
                            f"Stop process using port {port} or restart computer")
                        all_clear = False
                    else:
    ...
    def check_react_dependencies(self):
        ...
        if not lucide_path.exists():
            ...
            self.solutions.append(
                f"Run: cd \"{react_dir}\" && npm install lucide-react")
            return False
    ...
    def test_api_connectivity(self):
        ...
        try:
            response = requests.get(
                'http://localhost:3001/api/status', timeout=2)
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                if result == 0:
                    ...
                    print(
                        "❓ Streaming server not running (this is OK if system is not started)")
                    return True  # Not an error if system isn't running
        except Exception as e:
